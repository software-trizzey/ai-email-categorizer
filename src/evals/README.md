...
```sh
# Direct model/provider comparison
...
`eval:categorizer` calls providers directly from Promptfoo and is best for model/provider selection. `eval:categorizer:service` calls the local running service at `http://localhost:3000/eval/categorize` and is best for checking that the service code, prompt construction, parsing, and confidence gate still work together.
