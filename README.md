...
AI Email Categorizer is a Bun/Hono app that ingests inbound emails, classifies sender intent, and notifies admins when a message appears to request a quote or estimate.

See [`IDEA.md`](./IDEA.md) for the original system design notes.

```mermaid
flowchart LR
    email[Inbound email] --> webhook[Webhook endpoint]
    webhook --> orchestration[Orchestration service]
    orchestration -->|subject + body| categorization[Categorization service]
    categorization -->|JSON result + confidence| orchestration
    orchestration -->|alert if confidence meets threshold| notification[Notification service]
